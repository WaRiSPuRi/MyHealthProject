...
ILLNESS_DATABASE = {
    "cold": {
        ...
        "keywords": ["best medicine for cold"]
    },
    "flu": {
        "symptoms": ["fever", "chills", "body ache", "fatigue"],
        "keywords": ["best medicine for flu"]
    },
    "migraine": {
        "symptoms": ["headache", "nausea", "light sensitivity"],
        "keywords": ["best medicine for migraine"]
    }
}
...
# Function to get medicine recommendations using DuckDuckGo Search
def fetch_medicines(search_query):
    medicines = []
    try:
        with DDGS() as ddgs:
            results = list(ddgs.text(search_query + " site:healthline.com OR site:webmd.com", max_results=3))
            medicines = [res["title"] for res in results]
...
def get_diagnosis():
    ...
    if not symptoms:
        ...
        if illness:
            medicines = fetch_medicines(keywords[0])
